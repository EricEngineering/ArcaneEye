...
### macOS (the snip fix — read before touching capture/overlay)
macOS breaks the snip in two independent ways, both fixed with **`sys.platform == "darwin"` gates that leave the Windows / X11 / Wayland paths byte-for-byte unchanged** (the same "gate the platform-specific behavior" discipline as the Wayland section). Symptom before the fix: the whole screen went black on snip and *stayed* black.
