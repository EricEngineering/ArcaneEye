...
- **`arcaneeye/resources/`** — icons (`icon.png`/`.ico`, `trayicon.png`), `installer.ico`, and `AGPL_V3.txt` (shown in the license dialog). Loaded via `res_path`.
...
3. `CaptureScreen` covers the primary screen with a **40%-opacity black pixmap** (`createDimScreenEffect`), shows a `QRubberBand` on left-drag, and on mouse-release grabs the selected rectangle into `self.grabbedPixMap`. **`mouseReleaseEvent` branches three ways** by platform: **Wayland** → portal (`_grab_wayland`), **macOS** → `_grab_macos` (see the macOS section), **X11/Windows** → `_grab_x11` (`QGuiApplication.primaryScreen().grabWindow(0, x, y, w, h)`).
...
- A `QThread` running a `pynput.keyboard.GlobalHotKeys` listener, each hotkey emitting a Qt `Signal` back to `TrayApp` on the GUI thread. This is what makes the hotkeys work with no window focused. On exit, `TrayApp.exit_app()` stops it **cleanly** via `hotkey_thread.stop_listener()` (the pynput listener's `.stop()`) + `wait()`, with `terminate()`/`os._exit(0)` only as fallbacks — see Gotchas → *Clean shutdown of the hotkey listener* for why the old bare `terminate()` hung the app on macOS.
...
### Platform pipelines (overview — X11/Windows vs. Wayland vs. macOS)
The two OS-touching operations — **screen capture** and **full-screen window placement** (both the snip overlay and the Player View) — each fan out into three platform pipelines. Everything branches off two predicates: `is_wayland_session()` (`capture.py`) and `sys.platform == "darwin"`; **X11/Windows is the default/original path both others are gated away from.** The detail lives in the Wayland and macOS sections below; this is the map:

| Concern | X11 / Windows (default) | Wayland | macOS |
|---|---|---|---|
| **Screen grab** (`mouseReleaseEvent`) | `grabWindow(0,…)` direct (`_grab_x11`) | xdg-desktop-portal Screenshot over QDBus (`_grab_wayland`) | `grabWindow(0,…)` + Screen Recording permission (`_grab_macos`) |
| **Snip overlay** (`createDimScreenEffect`) | `setWindowState(WindowFullScreen)` | same (`WindowFullScreen`) | **cover by geometry** (`setGeometry`) — no native Space |
| **Grab timing** (`mouseReleaseEvent`) | `hide()` + `time.sleep(0.2)` | same | `hide()` + non-blocking `_wait_pumping()` |
| **Player View placement** (`_place_on_target`) | `showFullScreen()` | non-activating `setWindowState\|show` + hide-remap | **cover by geometry** (`setGeometry`+`show`) — no native Space |
| **Dismiss key** | `Esc` | `Ctrl+Shift+X` toggle | `Esc` |
| **Global hotkeys** (`pynput`) | direct | modifier-combo-only (see Wayland) | needs Accessibility permission |

The through-line: **Wayland can't read the framebuffer or position/lock windows the X11 way; macOS turns any real fullscreen into a separate Space that goes black on hide** — so both replace the two fullscreen paths and (Wayland) the grab. Keep new platform-specific behavior behind those same two gates so the default X11/Windows path stays untouched.
...
### macOS (the snip + display fixes — read before touching capture/overlay/placement)
macOS breaks the app in several independent ways, all fixed with **`sys.platform == "darwin"` gates that leave the Windows / X11 / Wayland paths byte-for-byte unchanged** (the same "gate the platform-specific behavior" discipline as the Wayland section). Two symptoms drove these: (a) the whole screen went black on snip and *stayed* black; (b) pressing Esc to hide the Player View turned the screen black. Both are the **native full-screen Space** trap — on macOS `showFullScreen()` / `setWindowState(WindowFullScreen)` doesn't borderless-maximize, it moves the window into a **separate, animated full-screen desktop (Space)**, where a frameless/translucent window renders as opaque black and tearing the Space down on hide leaves the screen black. The fix everywhere is the same: **cover the screen by geometry instead of entering the Space.**

**1. Overlay → don't use `WindowFullScreen`** (`createDimScreenEffect`). `setWindowState(Qt.WindowFullScreen)` on macOS triggers the **native full-screen Space** — a separate, animated desktop. A frameless, `WA_TranslucentBackground` overlay in that Space renders as **opaque black** (not a 40% dim over the live desktop), and tearing the Space down leaves the screen black. On macOS the overlay covers the screen by **geometry** instead (`setGeometry(primScreenGeo)`) — same dim, no Space, dismisses cleanly. Win/X11/Wayland keep `WindowFullScreen` (a plain borderless maximize there). **Consequence — the geometry overlay must be activated (`screenSnip` does `raise_()` + `activateWindow()` on macOS after `show()`):** geometry-`show()` (unlike the old `showFullScreen()`) does NOT make the window key, and a snip is usually launched by the global hotkey while another app is frontmost, so macOS wouldn't honor the crosshair cursor until the first click made the overlay key — the crosshair appeared late. Activating it up front fixes that. **Don't remove that `raise_()`/`activateWindow()`** thinking it's redundant — it's what makes the crosshair show immediately.

**2. Grab timing → no blocking `time.sleep`** (`mouseReleaseEvent`). The synchronous `time.sleep(0.2)` the other platforms use freezes the macOS window server so the overlay can't actually leave the screen before the grab — the black lingers. The macOS branch instead does `hide()` → **`_wait_pumping(250)`** (a nested `QEventLoop` quit by a one-shot `QTimer` — spins the loop so the desktop recomposites, without blocking the thread) → grab. The grab still completes inside `mouseReleaseEvent`, so `screenSnip`'s `hideEvent` handshake still delivers a valid pixmap. See `TODO(snip-wait)` in Gotchas about unifying this with the non-mac sleep later.

**3. Capture → Screen Recording permission (TCC)** (`capture.py`). `grabWindow(0, …)` works on macOS **only** with Screen Recording permission (System Settings → Privacy & Security → Screen Recording); without it macOS returns a **black** image rather than failing. `_grab_macos` calls **`macos_ensure_screen_permission()`** (`capture.py`), which preflights via CoreGraphics (`CGPreflightScreenCaptureAccess` through `ctypes` — **no pyobjc dependency**), fires the one-time system prompt (`CGRequestScreenCaptureAccess`, which also lists the app under Screen Recording), and if still not granted shows a **one-time** guidance dialog. **The grant is tied to the app's code signature/bundle identity and only takes effect after a relaunch** — so the *signed* release build makes it stick, while an unsigned dev run re-prompts. All these helpers are **no-ops returning `True` off macOS**, so nothing about the Win/Linux capture changes. Note this is a *different* permission from the **Accessibility** permission `pynput` needs for global hotkeys (Gotchas).

**4. Player View → cover by geometry, don't `showFullScreen()`** (`DisplayWindow._place_on_target`). This is the *display* side of the same Space trap: showing the Player View with `showFullScreen()` displayed fine, but the **Esc → `window.hide()`** teardown of the native Space turned the screen black. The macOS branch instead does `setScreen()` + `setGeometry(screen.geometry())` + `show()` + `raise_()` — same full-screen Player View on the target monitor, but no Space, so it hides cleanly. Win/X11 keep `showFullScreen()`; Wayland keeps its own non-activating `setWindowState|show` branch. (`WA_TransparentForMouseEvents` still makes it click-through, so it never traps the cursor on the player monitor.)

### Resource loading (dev vs frozen)
`res_path(name)` resolves bundled resources across three modes, in order: **PyInstaller** `sys._MEIPASS` (tries `arcaneeye/resources/<name>`, then legacy `resources/<name>`, then bundle root), **`importlib.resources`** over the `arcaneeye.resources` package, then a **dev source fallback** next to `main.py`. This mirrors ArcaneAtlas's `res_path`. The path constants (`icon_path`, `trayicon_path`) are computed once at import.
...
- **`time.sleep(0.2)` on the GUI thread** in `CaptureScreen.mouseReleaseEvent` blocks the event loop to let the dimming overlay repaint-away before the grab (so the 40% black doesn't end up in the screenshot). The author flagged it ("find a better way"). Any replacement must still guarantee the overlay is *actually* hidden on screen before `grabWindow` fires. **This blocking sleep is still used on the Windows / X11 / Wayland paths** (unchanged — they work). The **macOS path deliberately does NOT use it** (a synchronous sleep freezes the window server and leaves the screen black); it uses a non-blocking `_wait_pumping(ms)` (nested `QEventLoop` quit by a one-shot `QTimer`) instead. **TODO(snip-wait): investigate replacing the blocking `time.sleep(0.2)` on the non-macOS paths too** — the macOS `_wait_pumping` is a candidate unified approach, but must be tested on Windows + X11 + Wayland to confirm the overlay is reliably off-screen before the grab (the whole reason the sleep exists) before switching them over. Not doing it now.
...
- **Clean shutdown of the hotkey listener** (`exit_app`). Quitting does **not** `QThread.terminate()` the listener anymore — that abrupt kill was a **macOS hang bug**: pynput's `GlobalHotKeys` is itself a `threading.Thread` running a **Quartz `CFRunLoop`** on macOS, and `terminate()` killed only *our* `QThread`, leaving pynput's non-daemon run-loop thread alive → the process couldn't exit, so after the tray icon vanished the app lingered in the Dock as **"Application Not Responding"** (Force Quit required). Windows/X11 tore down fine, so it only bit on Mac. The fix (all platforms): `HotkeyListenerThread.stop_listener()` calls the pynput listener's **`.stop()`** (ends the platform run loop — `CFRunLoopStop` on macOS), which unblocks its `join()` so `run()` returns; `exit_app` then `wait(2000)`s on the thread, falls back to the old `terminate()` only if that times out, calls `QApplication.quit()`, and finally **`os._exit(0)`** as a belt-and-suspenders guarantee the process actually dies even if some background thread would otherwise keep it hung (safe here — the tray icon is hidden and the Qt loop is told to quit, so nothing worth unwinding remains). `run()` therefore no longer uses `with keyboard.GlobalHotKeys(...)` — it keeps the listener on `self` so `stop_listener()` can reach it. This likely also clears the "should not reach this on quit" noise in the THINGS TO DO list (same messy-teardown origin).
